...
module.exports._get = async (req, res, next) => {
    try {
        ...
        if (req.query.userAdmin_id) {
            aggregateQuery.push({ $match: { userAdmin_id: Number(req.query.userAdmin_id) } });
        }
        if (req.query.code) {
        ...
        let [userAdmins, counts] = await Promise.all([
            client.db(DB).collection(`UsersAdminEKT`).aggregate(aggregateQuery).toArray(),
            client
                .db(DB)
...
module.exports._getOne = async (req, res, next) => {
    try {
        // lấy data từ database
        let userAdmin = await client.db(DB).collection(`UsersAdminEKT`).findOne({ phone: req.params.userAdmin_phone });
        res.send({
            success: true,
            data: userAdmin,
        });
...
module.exports._create = async (req, res, next) => {
    try {
        let insert = await client.db(DB).collection(`UsersAdminEKT`).insertOne(req.body);
...
module.exports._update = async (req, res, next) => {
    try {
        ...
        let [userAdmin] = await client
            .db(DB)
            .collection(`UsersAdminEKT`)
            .aggregate([{ $match: { userAdmin_id: req.params.userAdmin_id } }])
            .toArray();
        delete userAdmin.password;
        let [accessToken] = await Promise.all([jwt.createToken(userAdmin, 24 * 60 * 60)]);
