...
export default function Sell() {
  ...
  const _addProductToCartInvoice = (product) => {
    ...
    if (product) {
      ...
      if (product.total_quantity !== 0) {
        ...
        invoicesNew[indexInvoice].vatListSum = invoicesNew[indexInvoice].vatList.reduce(
        ...
        invoicesNew[indexInvoice].moneyToBePaidByCustomer =
          ...
          invoicesNew[indexInvoice].vatListSum
    ...
    useEffect(() => {
      for (let i = 0; i < productsAllBranch.length; i++) {
        for (let j = 0; j < productsAllBranch[i].variants.length; j++) {
          const findVariant = productsAllBranch[i].variants.find(
  ...
  const _createOrder = async () => {
    try {
      ...
      const body = {
        ...
        list_tax: invoices[indexInvoice].vatList,
      }
  ...
  const _getTasList = async () => {
    try {
      const res = await getTaxs()
      if (res.status === 200) {
        setTaxList(res.data.data)
        if (invoices[indexInvoice].vatList.length === 0) {
          const taxDefault = res.data.data.find((e) => e.default)
          _editInvoice('vatList', taxDefault ? [taxDefault] : [])
        }
      }
    } catch (error) {
      console.log(error)
      ...
      <div className={styles['sell-content']}>
        ...
        <div className={styles['sell-right']}>
          ...
          <div>
            ...
            <Row wrap={false} align="middle">
              <div style={{ whiteSpace: 'nowrap' }}>Thuế áp dụng:</div>
              <Select
                allowClear
                ...
                value={invoices[indexInvoice].vatList.map((tax) => tax.tax_id)}
                onChange={(value) => {
                  const taxListNew = taxList.filter((tax) => value.includes(tax.tax_id))
                  _editInvoice('vatList', taxListNew)
                }}
              >
            ...
            <Row wrap={false} align="middle">
              <div style={{ width: '100%', marginBottom: 10 }}>
                {invoices[indexInvoice].vatList.map((tax) => (
                  <Row justify="space-between" wrap={false} align="middle">
                    <p style={{ marginBottom: 0 }}>
                      {tax.name} ({tax.value} %)
                    </p>
                    <p style={{ marginBottom: 0 }}>
                      {formatCash((tax.value / 100) * invoices[indexInvoice].sumCostPaid)}
                    </p>
                  </Row>
                ))}
              </div>
