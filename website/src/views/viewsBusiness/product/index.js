...
export default function Product() {
  ...
  const _getProductsToExport = async (query) => {
    try {
      const res = await getProducts({ branch_id: branchIdApp, branch: true, ...query })
  ...
  const ModalOptionExportProducts = () => {
    ...
    const handleExportProducts = async () => {
      setLoading(true)
      if (optionExport === 'all') await _getProductsToExport({ get_all: true })
          ...
          columns={columns.map((column) => {
            ...
            if (column.key === 'open_sell_quantity')
              return {
                    ...
                    <div style={{ color: 'gray' }}>
                      (
                      {
                        record.variants.filter(
                          (variant) => variant.locations && variant.locations.length
                        ).length
                      }{' '}
                      phiên bản)
                    </div>
                ...
                sorter: (a, b) => compare(a, b, 'open_sell_quantity'),
              }
              ...
              return {
                ...
                render: (text, record) => (
                  ...
                  >
                    ...
                    <div style={{ color: 'gray' }}>
                      ({record.variants.filter((variant) => variant.total_quantity).length} phiên
                      bản)
                    </div>
                  </div>
                ),
                sorter: (a, b) => compare(a, b, 'total_inventory'),
              }
